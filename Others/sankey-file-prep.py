...
taxonomic_ranks_last_linked_rank_dict = {
    ...
    "genus": 0,
    "species": 0}
# For value size testing; to be removed later
taxonomic_ranks_valuesize_dict = {
    "empty": 1,
    "domain": 1,
    "phylum": 0.1,
    "class": 0.01,
...
def hierarchy_counts(tax_filter, sample, average_samples, filename_combination):
    ...
    try:
        with open(biotavizfile, "r") as file:
            for _ in range(2):  # skip column headers + root
                next(file)
            taxonomic_rank = []
...
if __name__ == "__main__":
    ...
    average_all_samples = []
    sample = 0
    # biotavizfile = "biotaviz_maxsize_manualtest_2.txt"
    biotavizfile = "relative-table_kopie_test.biotaviz_test.txt"
